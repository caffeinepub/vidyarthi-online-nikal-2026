    ...
    <div className="space-y-5 animate-fade-in">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h1 className="text-xl font-bold text-foreground devanagari">Teacher Manage</h1>
            <p className="text-xs text-muted-foreground">शिक्षक लॉगिन व्यवस्थापन (Admin Only)</p>
          </div>
        </div>
        <Button onClick={openAdd} className="gradient-red text-white border-0 rounded-xl gap-2 hover:opacity-90">
          <Plus size={16} />
      ...
      <div className="bg-orange-50 border border-orange-200 rounded-xl p-4 text-sm text-orange-800 devanagari">
        ...
        <p>येथे नोंदणी केलेल्या मोबाईल नंबरने शिक्षक लॉगिन करू शकतात. प्रत्येक शिक्षकाचा मोबाईल नंबर आणि शाळेचा युडायस नंबर येथे जोडा.</p>
      </div>
          ...
          <Table>
            <TableHeader>
              <TableRow className="bg-muted/50">
                ...
                <TableHead className="devanagari font-semibold">मोबाईल नंबर</TableHead>
            ...
            <TableBody>
              {isLoading ? (
                <TableRow>
                  <TableCell colSpan={4} className="text-center py-8 text-muted-foreground">लोड होत आहे...</TableCell>
                </TableRow>
              ) : logins.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={4} className="text-center py-8 text-muted-foreground devanagari">
                ...
                logins.map(l => (
                  <TableRow key={l.id} className="hover:bg-muted/30">
                    <TableCell className="font-mono text-sm">{l.udise}</TableCell>
                    <TableCell className="devanagari">{getSchoolName(l.udise)}</TableCell>
        ...
        <DialogContent className="max-w-sm rounded-2xl">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <Input
                value={form.udise}
                onChange={e => setForm(f => ({ ...f, udise: e.target.value }))}
              ...
              <datalist id="manage-udise-list">
                {schools.map((s: SchoolExtra) => (
                  <option key={s.id} value={s.udise}>{s.name}</option>
                ))}
              </datalist>
              {form.udise && (
