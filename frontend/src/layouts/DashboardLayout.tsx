...
import {
  Home, School, Users, GraduationCap, ClipboardList,
  FileText, Printer, Settings, Menu, X, LogOut, ChevronRight
} from 'lucide-react';
...
const navItems: NavItem[] = [
  { key: 'home', label: 'Home', icon: <Home size={20} />, roles: ['admin', 'teacher'], color: 'text-orange-400' },
  { key: 'school-info', label: 'शाळा माहिती', icon: <School size={20} />, roles: ['admin', 'teacher'], color: 'text-blue-400' },
  { key: 'teacher-info', label: 'शिक्षक माहिती', icon: <Users size={20} />, roles: ['admin', 'teacher'], color: 'text-green-400' },
  { key: 'student-info', label: 'विद्यार्थी माहिती', icon: <GraduationCap size={20} />, roles: ['admin', 'teacher'], color: 'text-yellow-400' },
  { key: 'enter-result', label: 'निकाल भरा', icon: <ClipboardList size={20} />, roles: ['admin', 'teacher'], color: 'text-purple-400' },
  { key: 'progress-report', label: 'प्रगती पत्रक', icon: <FileText size={20} />, roles: ['admin', 'teacher'], color: 'text-pink-400' },
  { key: 'student-result-print', label: 'विद्यार्थी निकाल Print', icon: <Printer size={20} />, roles: ['admin', 'teacher', 'student'], color: 'text-cyan-400' },
  { key: 'teacher-manage', label: 'Teacher Manage', icon: <Settings size={20} />, roles: ['admin'], color: 'text-red-400' },
];
...
export default function DashboardLayout({ currentPage, onNavigate, children }: Props) {
  ...
  const visibleItems = navItems.filter(item => item.roles.includes(role as 'admin' | 'teacher' | 'student'));
    ...
    <div className="flex flex-col h-full sidebar-gradient">
        ...
        <div className="flex items-center gap-3">
          ...
          {!collapsed && (
            <div className="overflow-hidden">
              <p className="text-white font-bold text-sm devanagari leading-tight">विद्यार्थी निकाल</p>
      ...
      {!collapsed && (
        <div className="px-4 py-3 border-b border-white/10">
          <div className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold text-white ${badge.cls}`}>
            {badge.label}
          </div>
          ...
          >
            ...
            {!collapsed && (
              <>
                ...
                {currentPage === item.key && <ChevronRight size={14} className="text-white/40" />}
              </>
            )}
          </button>
        ))}
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="font-bold text-foreground text-sm devanagari">
                {visibleItems.find(i => i.key === currentPage)?.label || 'Dashboard'}
              </h2>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <span className={`hidden sm:inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold text-white ${badge.cls}`}>
              {badge.label}
            </span>
            <button
              onClick={logout}
