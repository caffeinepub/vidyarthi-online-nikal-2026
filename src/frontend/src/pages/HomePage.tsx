...
export default function HomePage() {
  ...
  const udiseFilter =
    session?.role === "teacher" && session.udise ? session.udise : null;
  ...
  return (
    <div className="space-y-6 animate-fade-in">
        ...
        {stats.map((stat) => (
          ...
          >
            <div className="flex items-start justify-between">
              <div>
                <p className="text-white/80 text-sm font-medium devanagari">
                  {stat.label}
                </p>
              ...
              <div className="bg-white/20 rounded-xl p-2.5">{stat.icon}</div>
            </div>
          </div>
        ))}
      </div>
      ...
      {filteredStudents.length > 0 && (
        <div className="bg-card rounded-2xl p-5 shadow-card border border-border">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h3 className="font-semibold text-foreground devanagari">
              निकाल प्रगती
            </h3>
          </div>
          <div className="space-y-2">
            <div className="flex justify-between text-sm text-muted-foreground">
              ...
              <span>
                {studentsWithResults.length} / {filteredStudents.length}
              </span>
            </div>
            <div className="w-full bg-muted rounded-full h-3 overflow-hidden">
              <div
                ...
                style={{
                  width: `${filteredStudents.length > 0 ? (studentsWithResults.length / filteredStudents.length) * 100 : 0}%`,
                }}
              />
            </div>
      ...
      {/* Quick Info */}
      <div className="bg-card rounded-2xl p-5 shadow-card border border-border">
        <h3 className="font-semibold text-foreground devanagari mb-3">
          📋 माहिती
        </h3>
