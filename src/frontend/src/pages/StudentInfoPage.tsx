...
export default function StudentInfoPage() {
  ...
  // Teachers see only their school's students; admins see all
  const teacherUdise = session?.role === "teacher" ? session.udise : null;
  const students = teacherUdise
    ? allStudents.filter((s: StudentExtra) => s.udise === teacherUdise)
    : allStudents;
  ...
  const filteredTeachers = teachers.filter(
    (t: TeacherExtra) => t.udise === form.udise,
  );
  ...
  const handleTeacherChange = (teacherName: string) => {
    const teacher = teachers.find(
      (t: TeacherExtra) => t.name === teacherName && t.udise === form.udise,
    );
    setForm((f) => ({
      ...f,
      teacherName,
  ...
  return (
    <div className="space-y-5 animate-fade-in">
      <div className="flex items-center justify-between">
        ...
        <Button
          onClick={openAdd}
          className="gradient-yellow text-white border-0 rounded-xl gap-2 hover:opacity-90"
          ...
          <Table>
            <TableHeader>
              <TableRow className="bg-muted/50">
                ...
                <TableHead className="devanagari font-semibold hidden lg:table-cell">
                  शिक्षक
                </TableHead>
              ...
              {isLoading ? (
                ...
                students.map((s) => (
                  <TableRow key={s.id} className="hover:bg-muted/30">
                    ...
                    <TableCell className="hidden lg:table-cell devanagari">
                      {s.teacherName}
                    </TableCell>
        ...
        <DialogContent className="max-w-md rounded-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="devanagari">
              {editItem ? "विद्यार्थी माहिती संपादित करा" : "नवीन विद्यार्थी जोडा"}
            </DialogTitle>
          </DialogHeader>
          <form onSubmit={handleSubmit} className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setForm((f) => ({ ...f, attendanceNo: e.target.value }))
                  }
            ...
            <DialogFooter className="pt-2">
              ...
              <Button
                type="submit"
                disabled={saving}
                className="gradient-yellow border-0 hover:opacity-90 devanagari"
                style={{ color: "oklch(0.18 0.04 260)" }}
              >
