...
interface SemesterSectionProps {
  ...
  onFieldChange: (
    field: "specialProgress" | "hobby" | "improvement",
    value: string,
  ) => void;
}
...
function SemesterSection({
  ...
  return (
    <div className="space-y-3">
      <div
        className={`p-3 rounded-xl text-white font-bold devanagari text-center ${sem === "semester1" ? "gradient-orange" : "gradient-green"}`}
      >
        {label}
      </div>
      <div className="space-y-2">
        {semData.subjects.map((subj, idx) => (
          <div key={subj.subject} className="flex items-center gap-2">
            <span className="flex-1 text-sm devanagari text-foreground">
              {subj.subject}
            </span>
            <Select
              value={subj.grade}
              onValueChange={(v) => onGradeChange(idx, v)}
            >
              ...
              <SelectContent>
                {GRADES.map((g) => (
                  <SelectItem key={g} value={g} className="devanagari">
                    {g}
                  </SelectItem>
                ))}
              </SelectContent>
      ...
      <div className="space-y-3 pt-2">
        <div>
          <Label className="devanagari text-xs font-semibold mb-1 block">
            ✨ विशेष प्रगती
          </Label>
          <textarea
...
export default function EnterResultPage() {
  ...
  // Teachers see only their school's data; admins see all
  ...
  const schools = teacherUdise
    ...
    : allSchools;
  const results = teacherUdise
    ? allResults.filter((r: ResultExtra) => r.udise === teacherUdise)
    : allResults;
  ...
  const filteredStudents = students.filter(
    (s: StudentExtra) => s.udise === form.udise,
  );
  ...
  const updateGrade = (
    ...
    setForm((f) => ({
      ...
      [sem]: {
        ...f[sem],
        subjects: f[sem].subjects.map((s, i) =>
          i === subjectIdx ? { ...s, grade } : s,
        ),
      },
    }));
  };
  ...
  return (
    <div className="space-y-5 animate-fade-in">
      <div className="flex items-center justify-between">
        ...
        <Button
          onClick={openAdd}
          className="gradient-purple text-white border-0 rounded-xl gap-2 hover:opacity-90"
        >
          <Plus size={16} />
          ...
          <Table>
            <TableHeader>
              <TableRow className="bg-muted/50">
                ...
                <TableHead className="devanagari font-semibold hidden sm:table-cell">
                  हजेरी नं.
                </TableHead>
                <TableHead className="devanagari font-semibold hidden sm:table-cell">
                  इयत्ता
                </TableHead>
                <TableHead className="devanagari font-semibold hidden md:table-cell">
                  तुकडी
                </TableHead>
                <TableHead className="devanagari font-semibold hidden md:table-cell">
                  युडायस
              ...
              {isLoading ? (
                ...
                results.map((r) => (
                  <TableRow key={r.id} className="hover:bg-muted/30">
                    <TableCell className="font-medium devanagari">
                      {r.studentName}
                    </TableCell>
                    <TableCell className="hidden sm:table-cell">
                      {r.attendanceNo}
                    </TableCell>
                    <TableCell className="hidden sm:table-cell devanagari">
                      {r.className}
                    </TableCell>
                    <TableCell className="hidden md:table-cell devanagari">
                      {r.division}
                    </TableCell>
        ...
        <DialogContent className="max-w-lg rounded-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="devanagari">
              {editItem ? "निकाल संपादित करा" : "नवीन निकाल भरा"}
            </DialogTitle>
          </DialogHeader>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="bg-muted/30 rounded-xl p-3 space-y-3">
              <div>
                ...
                <Select
                  value={form.udise}
                  onValueChange={handleUdiseChange}
              ...
              <div className="grid grid-cols-3 gap-2">
                <div>
                  ...
                  <Input
                    ...
                    readOnly
                    className="bg-muted text-sm h-8 devanagari"
                  />
                </div>
                <div>
            ...
            <SemesterSection
              ...
              onGradeChange={(idx, grade) =>
                updateGrade("semester1", idx, grade)
              }
              onFieldChange={(field, value) =>
                updateSemField("semester1", field, value)
              }
            />
            ...
            <SemesterSection
              ...
              onGradeChange={(idx, grade) =>
                updateGrade("semester2", idx, grade)
              }
              onFieldChange={(field, value) =>
                updateSemField("semester2", field, value)
              }
            />
            ...
            <DialogFooter className="pt-2">
              ...
              <Button
                ...
                className="gradient-purple text-white border-0 hover:opacity-90"
              >
