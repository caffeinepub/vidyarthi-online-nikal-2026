...
import type { SchoolExtra, TeacherExtra } from "../lib/localStorage";
...
export default function TeacherInfoPage() {
  ...
  // Teachers see only their school's teachers; admins see all
  const teacherUdise = session?.role === "teacher" ? session.udise : null;
  const teachers = teacherUdise
    ? allTeachers.filter((t: TeacherExtra) => t.udise === teacherUdise)
    : allTeachers;
  ...
  return (
    <div className="space-y-5 animate-fade-in">
      <div className="flex items-center justify-between">
        ...
        <Button
          onClick={openAdd}
          className="gradient-green text-white border-0 rounded-xl gap-2 hover:opacity-90"
        >
          <Plus size={16} />
        ...
        <DialogContent className="max-w-md rounded-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="devanagari">
              {editItem ? "शिक्षक माहिती संपादित करा" : "नवीन शिक्षक जोडा"}
            </DialogTitle>
          </DialogHeader>
          <form onSubmit={handleSubmit} className="space-y-3">
            ...
            <DialogFooter className="pt-2">
              ...
              <Button
                type="submit"
                disabled={saving}
                className="gradient-green text-white border-0 hover:opacity-90"
              >
